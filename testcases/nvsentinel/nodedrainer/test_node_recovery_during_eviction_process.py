...
#     """
#     Class for test case of NVsentinel Fault Notification: Node recovery during pod eviction process before the pod is shifted to another node
#     """
...
#                 )
    ...
    # def test_node_recovery_during_eviction_process(self, request):
    #     """
    #     Test case of NVsentinel Fault Notification: Node recovery during pod eviction process before the pod is shifted to another node
    #     """
    #     self.step_manager.print_header("Check the fault-notification pod is running")
        # Get fault notification pod
...
#             os.getcwd(), "data", "cli", "nvsentinel", "allowcompletion-immediate-mode.yaml"
#         )
...
#         )
#         self.logger.info(f"MPI training job pod is running on node: {self.node_name}")
...
#             expected_messages=[
#                 f"Evicting pods from namespace {self.default_namespace} in AllowCompletion mode",
#                 "Evicting pods from namespace busybox in Immiediate mode",
#             ]
#         )

#         self.step_manager.print_header("Check busybox pods are running")
#         filtered_pod_names = [
...
#         # Verify all busybox pods are running
#         self.pod_info.verify_pods_are_running_by_kubectl(
#             pod_names=filtered_pod_names, namespace="busybox", timeout=300
#         )

#         self.step_manager.print_header("The job pod is still running on the original node")
...
#         self.step_manager.print_header("Checking the fault-notification pod log again")
#         self.verify_fault_notification_pod_log(
#             expected_messages=[
#                 f"Cancelling the eviction of pods in namespace {self.default_namespace} on node"
#             ]
#         )
#         self.step_manager.print_header("Check the pods again")
#         self.pod_info.verify_pods_are_running_by_kubectl(
#             pod_names=filtered_pod_names, namespace="busybox", timeout=300
#         )
...
#         self.device.runai.training.mpi.delete(name=self.job_name, project=self.project)
