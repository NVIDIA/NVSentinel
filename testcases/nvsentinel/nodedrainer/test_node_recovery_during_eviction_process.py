...
class TestNodeRecoveryDuringEvictionProcess(TestNVSentinelCaseBase):
    ...
    def test_node_recovery_during_eviction_process(self, request, nvsentinel_autosync_disabled_enabled):
        """
        Tests AllowCompletion mode: verifies that:
            1. The node is cordoned on injection of gpu fatal error
            2. Job pod is not evicted and is running on the original node due to AllowCompletion mode
            3. busybox pods are evicted due to immediate mode
            4. The node is recovered when the error is cleared
        """
        ...
        self.client.verify_pod_are_running([node_drainer_pod.metadata.name], self.nv_namespace)
       ...
       # Apply busybox pods using the Python SDK
        ...
        self.client.verify_pod_are_running(busybox_pod_names, "busybox")

        self.step_manager.print_header(
            "Pick one node and inject a gpu fatal error on the node, eg node 10.0.43.235 , where are busybox-2 and training job are running"
        )
        ...
        # Verify all busybox pods are running
        self.client.verify_pod_are_running(filtered_pod_names, "busybox")
