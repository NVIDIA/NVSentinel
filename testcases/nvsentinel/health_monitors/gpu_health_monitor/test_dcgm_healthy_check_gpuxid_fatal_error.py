...
"""
Module for class of NVsentinel GPU Health Monitor: DCGM Healthy check GpuXid fatal error
"""
...
class TestDGCMHealthyCheckGpuXidFatalError(GPUHealthMonitorBase):
    """
    Class for test case of NVsentinel GPU Health Monitor: DCGM Healthy check GpuXid fatal error
    """

    @pytest.mark.author(email="ajmishra@nvidia.com")
    @pytest.mark.gpuhealthmonitor
    def test_dcgm_healthy_check_gpuxid_fatal_error(self, request):
        """
        Tests if node condition are set correctly when a fatal xid error is injected from GPU health monitor
        """
        ...
        self.step_manager.print_header(
            "Exec into one gpu health monitor pod and inject a xid error"
        )
        command = [
            "/bin/sh",
            "-c",
             f"dcgmi test --host nvidia-dcgm.{self.gpu_operator_namespace}.svc:5555 --inject --gpuid 0 -f 230 -v 46",
        ]
