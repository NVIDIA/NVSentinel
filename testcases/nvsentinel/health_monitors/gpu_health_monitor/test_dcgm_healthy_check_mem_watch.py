...
"""
Module for class of NVsentinel GPU Health Monitor: DCGM Healthy check MemWatch
"""
...
class TestDGCMHealthyCheckMemWatch(GPUHealthMonitorBase):
    """
    Class for test case of NVsentinel GPU Health Monitor: DCGM Healthy check MemWatch
    """

    @pytest.mark.author(email="ajmishra@nvidia.com")
    @pytest.mark.gpuhealthmonitor
    def test_dcgm_healthy_check_mem_watch(self, request):
        """
        Tests if the node condition GpuMemWatch is updated correctly when a MemWatch error is injected from GPU health monitor
        """
        ...
        self.step_manager.print_header(
            "Exec into one gpu health monitor pod and inject a MemWatch error"
        )
        command = [
            "/bin/sh",
            "-c",
             f"dcgmi test --host nvidia-dcgm.{self.gpu_operator_namespace}.svc:5555 --inject --gpuid 0 -f 395 -v 1",
        ]
