...
type K8sConnectorConfig struct {
	...
	// UpdateOnlyOnChange makes a batch cost API calls only when it changes what
	// the cluster shows: the node status update is skipped when the condition
	// would keep its status, reason and message, and the Kubernetes Event
	// write is skipped for a fault whose Event was written less than
	// nodeEventRefreshInterval ago. The deployment platform connector sets it:
	// there a repeat, including a resent batch, must not cost an API call. The
	// node-local DaemonSet keeps today's behavior: every batch refreshes the
	// condition heartbeat and bumps the count of a repeated fault's Event.
	UpdateOnlyOnChange bool
