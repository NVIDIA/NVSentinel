...
func evaluate(t *testing.T, expression string, event *pb.HealthEvent) bool {
	...
	program, err := CompileBool(env, expression)
	require.NoError(t, err)

	matched, err := EvaluateBool(program, event)
	require.NoError(t, err)
...
func TestEvaluateBool_FieldExpressions_MatchTheEventsValues(t *testing.T) {
	tests := []struct {
		...
		{"isFatal is a bool", `event.isFatal`, true},
		{"isHealthy is a bool", `!event.isHealthy`, true},
...
func TestEvaluateBool_ErrorCode_BindsAsAListNotAString(t *testing.T) {
	...
	program, err := CompileBool(env, `'145.RLW_SRC_TRACK' in event.errorCode`)
	require.NoError(t, err)
...
func TestCompileBool_ConcreteNonBooleanType_IsRejectedAtCompileTime(t *testing.T) {
	...
	_, err = CompileBool(env, `1 + 1`)

	require.Error(t, err)
	assert.Contains(t, err.Error(), "must return boolean")
}

func TestCompileBool_FieldReadingExpression_CompilesEvenWhenNotBoolean(t *testing.T) {
	// "event" is map[string]dyn, so every field read is statically dyn. That is why
	// `event.isFatal` has to be accepted, and the cost is that `event.agent` cannot be
	// rejected until evaluation. Asserted so the limit of startup validation is explicit.
	env, err := NewEnv()
	require.NoError(t, err)

	program, err := CompileBool(env, `event.agent`)
	require.NoError(t, err, "a dyn-typed expression compiles")

	_, err = EvaluateBool(program, testEvent())
	require.Error(t, err)
...
func TestCompileBool_MalformedExpression_IsRejected(t *testing.T) {
	...
	_, err = CompileBool(env, `event.agent ==`)
...
func TestCompileBool_UnknownField_IsRejectedAtCompileTime(t *testing.T) {
	...
	program, err := CompileBool(env, `event.notAField == 'x'`)
