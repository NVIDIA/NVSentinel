...
Note: there is one class of bad input the layered validation cannot catch — `customRecommendedAction` values not registered in fault-remediation's TOML config. The webhook lives in a different process from fault-remediation and has no access to that config, so it cannot validate the action name's correctness at admission. EFs carrying such values flow through admission, get emitted as health events, and are silently skipped by fault-remediation's lookup ("Action not found in remediation configuration") — leaving the EF stuck at `FaultReported=Unknown`. Operators rely on `ef_open` persistence alerts (and fault-remediation's own warning log) to detect these cases. Mitigation is procedural: register custom actions in fault-remediation's TOML before any external system starts emitting them. This is the same coupling that already exists for ADR-036 custom actions.
...
| `ef_total` | Counter | `phase` (`created`, `reported`, `cleared`); `result` (`success`, `failure`; empty when `phase=created`) | EFs entered each phase. `reported` with `result=failure` covers the Layer-3-safety-net rejection path (`FaultReported=False, reason=InvalidHealthEvent`); `cleared` with `result=failure` covers the `ExternalRemediationFailed` terminal state (`FaultCleared=False`). |
...
| `ef_open` | Gauge | `node`, `recommended_action` | Open EFs — `FaultReported=Unknown`, the EF reconciler has submitted (or is still retrying to submit) the HealthEvent but no owning ERR has been observed yet. Rising values typically indicate blocked equivalence-group supersedence (a maintenance CR in flight on the same node) or an unregistered `customRecommendedAction`. |
...
| `ef_age_seconds` | Histogram | `recommended_action`, `result` (`success`, `failure`) | Time from EF creation to terminal `FaultCleared`. Tracked separately for success (clean recovery — node returned to NVSentinel ownership) and failure (`ExternalRemediationFailed` — node remains quarantined awaiting retry or operator action) resolutions. |
| `health_event_emit_failures_total` | Counter | `source` (`ef-reconciler`); `kind` (`fault`, `recovery`) | gRPC failures emitting to platform-connector. `kind=fault` is the initial CUSTOM HealthEvent submission triggered by EF reconciliation; `kind=recovery` is the healthy companion event emitted on successful completion. Separating them lets operators distinguish "external system can't get a fault into the pipeline" (severe — admission path failing) from "external system completed but recovery announcement is failing" (less severe — the system self-corrects on the next monitor scrape). |
| `ef_webhook_rejections_total` | Counter | `reason` (one of: `disabled`, `bad_health_event`, `node_not_found`, `duplicate_ef`) | EF create/update rejections by the validating webhook. Per-reason rates are operationally distinct: `bad_health_event` suggests a misbehaving external system; `duplicate_ef` suggests an external system that isn't honoring terminal-state semantics; `disabled` suggests an unexpected toggle of the controller-enabled flag; `node_not_found` typically signals a race between cluster scale-down and external-system submission. |
...
In addition to the metrics above, both reconcilers expose `controller_runtime_reconcile_errors_total{controller}` and `controller_runtime_reconcile_total{controller, result}` for free via the controller-runtime library — operators get reconcile-loop error rates and per-controller throughput without further plumbing. The standard kubebuilder dashboards work as-is.

**Events** (Kubernetes events on the EF / ERR object):

- ERR: `ReleaseTaintApplied`, `ReleaseTaintFailed`, `ReleaseTaintRemoved`, `OwnershipPropagated`
- EF: `ERRObserved` (the `FaultReported=Unknown→True` transition), `HealthEventEmitted`, `HealthyEventEmitted`, `EmitFailed`, `ExternalRemediationFailed` (the `FaultCleared=False` terminal case)
