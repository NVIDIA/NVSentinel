...
MetadataAugmentor looks up the target node's labels from an LRU cache (backed by Kubernetes API calls with a short TTL, default 30 seconds) and checks for the configured skip label (`nvsentinel.dgxc.nvidia.com/managed=false`). When matched, the event's `ProcessingStrategy` is set to `STORE_ONLY` before it reaches the Kubernetes connector, datastore, or fault-quarantine. This means the event is recorded for audit but does not trigger remediation, Kubernetes side-effects, or analysis.
...
- **Cache staleness:** Up to the cache TTL (default 30s) of delay in both directions: when `managed=false` is applied, and when it is removed. During this window, events may be processed normally for an opted-out node, or dropped for a node that has been returned to NVSentinel ownership.
