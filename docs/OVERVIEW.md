...
**Preflight** (optional) is not part of the health-event pipeline. A mutating webhook injects GPU diagnostic init containers at pod admission so bad hardware is caught before the workload starts. Multi-node checks can use **gang discovery** (native Kubernetes PodGroup API or PodGroup-style schedulers) and ConfigMap coordination. See [Preflight configuration](./configuration/preflight.md) and [ADR-026](./designs/026-preflight-checks.md).
