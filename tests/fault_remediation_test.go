...
// TestVariousRemediationActions verifies different action types all create rebootnode CRs
func TestVariousRemediationActions(t *testing.T) {
	feature := features.New("TestVariousRemediationActions").
		WithLabel("suite", "fault-remediation")
	...
	feature.Assess("various remediation actions all create rebootnode CR", func(ctx context.Context, t *testing.T, c *envconf.Config) context.Context {
		client, err := c.NewClient()
		require.NoError(t, err)

		// Test restart BM action (24)
		t.Log("Testing restart BM action")
		helpers.TriggerFullRemediationFlow(ctx, t, client, testCtx.NodeName, 24)
		cr := helpers.WaitForRebootNodeCR(ctx, t, client, testCtx.NodeName)
		require.NotNil(t, cr)
		helpers.WaitForNodeRemediationLabel(ctx, t, client, testCtx.NodeName, helpers.RemediationSucceededLabelValue)

		// Cleanup and test restart VM action (15)
		t.Log("Cleaning up and testing restart VM action")
		helpers.SendHealthyEvent(ctx, t, testCtx.NodeName)
		helpers.CleanupRebootNodeCRs(ctx, t, client)
		time.Sleep(5 * time.Second)
		...
		// Cleanup and test replace VM action (25)
		t.Log("Cleaning up and testing replace VM action")
		helpers.SendHealthyEvent(ctx, t, testCtx.NodeName)
		helpers.CleanupRebootNodeCRs(ctx, t, client)
		time.Sleep(5 * time.Second)

		helpers.TriggerFullRemediationFlow(ctx, t, client, testCtx.NodeName, 25)
		cr = helpers.WaitForRebootNodeCR(ctx, t, client, testCtx.NodeName)
		require.NotNil(t, cr)
