...
func createStandardizedFactory(config DatastoreClientConfig) (*factory.ClientFactory, error) {
	...
	if config.DataStoreConfig != nil {
		...
		if config.DatabaseClientCertMountPath != "" {
			...
			legacyConfig := adapter.ConvertDataStoreConfigToLegacyWithCertPath(config.DataStoreConfig, config.DatabaseClientCertMountPath)
...
func NewChangeStreamWatcherOnly(ctx context.Context, moduleName string, pipeline interface{}) (client.ChangeStreamWatcher, error) {
	config := DatastoreClientConfig{
		ModuleName: moduleName,
		Pipeline:   pipeline,
	}
...
func NewDatastoreClientFromConfig(ctx context.Context, moduleName string, dsConfig datastore.DataStoreConfig, pipeline interface{}) (*DatastoreClientBundle, error) {
	config := DatastoreClientConfig{
		ModuleName:      moduleName,
...
func NewDatastoreClientWithCertPath(ctx context.Context, moduleName string, certMountPath string, pipeline interface{}) (*DatastoreClientBundle, error) {
	config := DatastoreClientConfig{
		ModuleName:                  moduleName,
