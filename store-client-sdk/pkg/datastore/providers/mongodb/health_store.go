...
// InsertMany inserts multiple health events at once
func (h *MongoHealthEventStore) InsertMany(ctx context.Context, events []datastore.HealthEventWithStatus) error {
	if len(events) == 0 {
		return nil // Nothing to insert
	}

	// Convert to interface slice for the client interface
	documents := make([]interface{}, len(events))
	for i, event := range events {
		documents[i] = event
	}

	// Use bulk insert functionality - this method needs to be implemented in the DatabaseClient
	// For now, we'll implement it as individual upserts
	for _, event := range events {
		filter := map[string]interface{}{
			"healthevent.nodename": event.HealthEvent,
		}

		_, err := h.databaseClient.UpsertDocument(ctx, filter, event)
		if err != nil {
			return datastore.NewInsertError(
				datastore.ProviderMongoDB,
				"failed to insert health event in bulk",
				err,
			).WithMetadata("operation", "bulk_insert")
		}
	}
