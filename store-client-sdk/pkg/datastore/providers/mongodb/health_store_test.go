...
func TestMongoHealthEventStore_InsertMany(t *testing.T) {
	ctx := context.Background()
	mockDB := new(MockDatabaseClient)
	store := &MongoHealthEventStore{
		databaseClient: mockDB,
	...
	t.Run("empty slice", func(t *testing.T) {
		err := store.InsertMany(ctx, []datastore.HealthEventWithStatus{})
		assert.NoError(t, err)
	})
	...
	t.Run("successful insert", func(t *testing.T) {
		...
		// Should call UpsertDocument for each event
		for range events {
			mockDB.On("UpsertDocument", ctx, mock.AnythingOfType("map[string]interface {}"), mock.AnythingOfType("datastore.HealthEventWithStatus")).Return(&client.UpdateResult{}, nil)
		}

		err := store.InsertMany(ctx, events)
		assert.NoError(t, err)

		mockDB.AssertExpectations(t)
	})

	// Test error during insert
	t.Run("error during insert", func(t *testing.T) {
		...
		mockDB.On("UpsertDocument", ctx, mock.AnythingOfType("map[string]interface {}"), mock.AnythingOfType("datastore.HealthEventWithStatus")).Return((*client.UpdateResult)(nil), errors.New("insert error"))

		err := store.InsertMany(ctx, events)
		assert.Error(t, err)
		assert.Contains(t, err.Error(), "failed to insert health event in bulk")
