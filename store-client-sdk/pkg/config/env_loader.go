...
func NewDatabaseConfigForCollectionType(certMountPath, collectionType string) (DatabaseConfig, error) {
	...
	// This encapsulates MongoDB-specific knowledge within the store-client-sdk
...
func loadTimeoutConfigFromEnv() (TimeoutConfig, error) {
	...
	changeStreamRetryDeadline, err := getPositiveIntEnvVar(EnvChangeStreamRetryDeadlineSeconds,
		DefaultChangeStreamRetryDeadline)
	if err != nil {
		return nil, fmt.Errorf("invalid %s: %w", EnvChangeStreamRetryDeadlineSeconds, err)
	}

	changeStreamRetryInterval, err := getPositiveIntEnvVar(EnvChangeStreamRetryIntervalSeconds,
		DefaultChangeStreamRetryInterval)
	if err != nil {
		return nil, fmt.Errorf("invalid %s: %w", EnvChangeStreamRetryIntervalSeconds, err)
	}
