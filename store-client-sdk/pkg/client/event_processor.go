...
func NewEventProcessor(watcher ChangeStreamWatcher, dbClient DatabaseClient, config EventProcessorConfig) EventProcessor {
...
func (p *DefaultEventProcessor) handleSingleEvent(ctx context.Context, event Event) error {
	...
	if processErr != nil {
		...
		// Don't return error here - we still want to mark as processed to avoid stuck streams
	}
